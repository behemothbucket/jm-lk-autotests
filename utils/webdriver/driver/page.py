...
class Page(PageInterface):
    ...
    def wait_for_correct_current_url(
        ...
        try:
            self.wait(timeout).until(
                lambda _: self.url() == self.config.base_url + expected_url,
            )
    ...
    def visit(self, url: str) -> "Page":
        ...
        logger.info("Page.visit() - Открыть URL: `%s`", normalized_url)
    ...
    def switch_to_tab_action_close_tab(self, url: str, callback, **kwargs) -> None:
        ...
        if callback:
            logger.info(f"Выполнение callback: {callback.__name__}")
